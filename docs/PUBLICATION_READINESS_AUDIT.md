...
This audit separates launch readiness from strict publication certification. The app can already run complete local studies, generate templates, validate uploads, compute all built-in methods, link matched DOI examples where available, and export publication-oriented material. The remaining strict-certification risk is external numerical validation depth for methods that still need a first matched DOI fixture.
...
- Methods with matched fixtures: 60/65.
- Methods needing a first matched DOI fixture: 5 (SECA, CRADIS, RAPS, WEDBA, COBRA).
- Validation candidate methods: 5.
- Candidate records needing reconciliation: 5.
- Internal coverage only: 0 methods.
- Launch-ready with evidence boundary: yes.
- Strict publication certification complete: no.
...
None of the original internal-only method fixtures remain in this category. Remaining blockers are validation candidates and discrepancy records, not unvalidated original internal-only methods.
...
4. COBRA registry-only candidate - Financial Innovation 2025 publishes an ITARA-COBRA financial-performance case with COBRA table evidence; the transformed input matrix, criteria weights, direction convention, and final ranking tables still need extraction before promotion.

Candidate-discrepancy records also remain for TOPSIS variant comparisons, CRADIS score-convention reconciliation, and fuzzy DEMATEL convention matching. TOPSIS and DEMATEL already have separate passing fixtures, so these extra discrepancy records should be shown as audit notes, not as blockers for their existing matched-method labels. CRADIS still needs either score-convention reconciliation or a separate active fixture before it receives a first matched-method label.
...
MCDM Studio is a local-first MCDM research workbench with 65 implemented methods, 28 weighting modes, native triangular/trapezoidal fuzzy workflows, group/respondent support, method-specific Excel templates, upload validation, intermediate calculation tables, visualizations, Excel/DOCX/PDF/JSON exports, and 60 methods with at least one reproduced DOI or official worked-example fixture.
