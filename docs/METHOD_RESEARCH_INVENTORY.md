...
- DEAR crisp CNC milling multi-response optimization: material removal rate benefit response, surface roughness cost response, response weights, MRPI ratio scoring, and final experiment ranking from Materials 2022.
- EAMR crisp internal-grinding dressing-process selection: benefit/non-benefit normalized values, weighted G+ and G- components, score ratio, and final ranking from Machines 2022.
- RAWEC crisp double-normalization agricultural distribution-center selection: raw decision matrix, benefit/cost directions, LMAW-derived manual weights, first and second normalized matrices, weighted deviations, Q deviation index, and final ranking from the MethodsX 2024 original RAWEC paper.
- ESP-SPOTIS crisp used-car expected solution point selection: mileage, price, and year criteria with manual bounds, expected solution point, RANCOM-derived weights, ESP-only SPOTIS preference values, and final ranking from the ICAART 2025 B-SPOTIS paper.
- MACONT crisp sustainable third-party reverse logistics provider selection: comprehensive normalization, average virtual reference provider, mixed aggregation scores, and final ranking from the Informatica 2020 original MACONT paper.
...
- B-SPOTIS crisp used-car ISP/ESP case: ICAART 2025 paper fixture with mileage, price, and year criteria; RANCOM-derived weights; manual bounds; expected solution point; ISP/ESP distance scores; alpha 0.5 balanced scores; and final used-car ranking.
...
- COBRA crisp financial-performance case: the Financial Innovation 2025 ITARA-COBRA BIST 100 example exposes COBRA table/ranking evidence, but the transformed decision matrix, criteria weights, and direction convention still need extraction before a passing executable fixture can be added.
