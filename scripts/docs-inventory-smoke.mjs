...
const activeFixtureMethodIds = new Set([...activeFixtureBlock.matchAll(/methodId:\s*'([^']+)'/g)].map((match) => match[1]));
...
check('Fixture inventory lists registered B-SPOTIS example', readFileSync('docs/external-fixtures/README.md', 'utf8').includes('balanced-spotis-crisp-pymcdm-2026.json') && roadmap.includes('B-SPOTIS crisp alpha-balanced') && researchInventory.includes('B-SPOTIS crisp manual-bounds ESP/ISP audit case'));
...
check('Fixture inventory lists discrepancy candidates', readFileSync('docs/external-fixtures/README.md', 'utf8').includes('cradis-crisp-electric-vehicle-jaes-2025.discrepancy.json') && readFileSync('docs/external-fixtures/README.md', 'utf8').includes('topsis-crisp-warehouse-storage-springer-2026.discrepancy.json') && readFileSync('docs/external-fixtures/README.md', 'utf8').includes('topsis-crisp-etl-software-springerplus-2016.discrepancy.json') && readFileSync('docs/external-fixtures/README.md', 'utf8').includes('topsis-crisp-barge-service-sciencedirect-2025.discrepancy.json') && roadmap.includes('TOPSIS, CRADIS, and B-SPOTIS discrepancy/candidate records') && researchInventory.includes('CRADIS crisp ratio-normalization electric-vehicle') && researchInventory.includes('TOPSIS crisp vector-normalization warehouse-storage') && researchInventory.includes('TOPSIS crisp vector-normalization ETL software-selection') && researchInventory.includes('TOPSIS crisp vector-normalization barge-service') && researchInventory.includes('B-SPOTIS crisp used-car case'));
...
check(
  ...
  validationMatrix.includes(`Passing external fixtures: ${activeFixtureMethodIds.size} methods/variants`) &&
    validationMatrix.includes(`Validation candidates: ${candidateMethodCount} methods, ${candidateFixtureCount} candidate records`) &&
...
check('Capability matrix explains group and fuzzy strategies', capabilityMatrixSource.includes('groupDecisionCapability') && capabilityMatrixSource.includes('fuzzyCapability') && capabilityMatrixSource.includes('validationBoundary') && capabilityMatrixSource.includes('geometric mean') && capabilityMatrixSource.includes('centroid defuzzification'));
check('Readiness UI displays capability profile', readFileSync('src/main.tsx', 'utf8').includes('capabilityProfile') && readFileSync('src/main.tsx', 'utf8').includes('Respondent strategy') && readFileSync('src/main.tsx', 'utf8').includes('Fuzzy strategy'));
